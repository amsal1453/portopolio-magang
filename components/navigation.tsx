...
export default function Navigation() {
  ...
  return (
    <motion.nav
      className="fixed top-0 left-0 right-0 z-40 bg-slate-950/80 border-b border-slate-800 backdrop-blur-md transition-colors duration-300"
      initial={{ y: -100 }}
      animate={{ y: 0 }}
        ...
        <div className="px-2 pt-2 pb-3 space-y-1 bg-slate-900/95 backdrop-blur-md">
          {navItems.map((item) => (
            <motion.button
              key={item.name}
              onClick={() => scrollToSection(item.href)}
              className={`block px-3 py-2 rounded-md text-base font-medium w-full text-left transition-colors ${activeSection === item.href.substring(1)
